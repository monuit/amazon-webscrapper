...
class Scraper():
    ...
    def generate_output_file(self):
        """Generates JSON and Parquet files from the list of products found in the whole search."""
    
        products_json_list = []
        # Generate random file name
        json_filename = str(uuid.uuid4()) + '.json'
        parquet_filename = str(uuid.uuid4()) + '.parquet'
    
        # Convert every object into JSON format
        ...
        product_data_list = [json.loads(product_json) for product_json in products_json_list.split(",")]
